...
class AudioService {
    constructor() {
        ...
        this.oauthFile = path.join(__dirname, 'oauth.json');
        this.ytdlpCookiesFile = path.join(this.dataDir, 'yt-dlp-cookies.txt');
    ...
    getYtdlpCookieArgs() {
        ...
        const cookiesFile = this.writeYtdlpCookiesFromOauth();
        return cookiesFile ? ['--cookies', cookiesFile] : [];
    }

    writeYtdlpCookiesFromOauth() {
        try {
            if (!fs.existsSync(this.oauthFile)) return null;
            const raw = fs.readFileSync(this.oauthFile, 'utf8');
            const parsed = JSON.parse(raw);
            const cookieHeader = parsed.cookie || parsed.Cookie;
            ...
            const content = [
                ...
                '# Generated by Event Jukebox from oauth.json for yt-dlp.',
                ...cookieLines
            ].join('\n') + '\n';

            fs.writeFileSync(this.ytdlpCookiesFile, content, { mode: 0o600 });
            fs.chmodSync(this.ytdlpCookiesFile, 0o600);
            return this.ytdlpCookiesFile;
        } catch (error) {
            console.log('⚠️ Failed to prepare yt-dlp cookies:', error.message);
            return null;
        }
    }
