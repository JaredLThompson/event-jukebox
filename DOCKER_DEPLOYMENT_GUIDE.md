...
- **Local scripts**: `wedding-jukebox` (image `wedding-jukebox:latest`)
- **Pi Docker setup**: `wedding-jukebox-pi` (image `ghcr.io/jaredlthompson/wedding-jukebox:latest`)

To avoid conflicts, use **one** approach at a time. On the Pi, prefer the systemd Docker service (`wedding-jukebox-docker`) and the `wedding-jukebox-pi` container.
...
```bash
docker exec -it wedding-jukebox-pi aplay -l
```
...
```
Stop the process or avoid running both Docker and the legacy `wedding-jukebox` service at the same time.
